...
${error.stack}`);
    ...
    function collapseTopicSummaryView(topicId, options = {}) {
      ...
      if (clearTracking && getTrackedExpandedTopicId() === normalized) {
        setTrackedExpandedTopicId(null);
      }
      return true;
    }
    function collapseAllTopicSummaryViewsExcept(topicId = null, options = {}) {
      ...
      if (keepTopicId) {
        setTrackedExpandedTopicId(keepTopicId);
    ...
    function expandTopicSummaryView(topicId, options = {}) {
      ...
      if (!summaryRow || !container || !buttonGroup) {
        if (track && !preserveTrackingOnFailure) setTrackedExpandedTopicId(null);
        return false;
      }
    ...
    function addTopicListSummaryButtons() {
      ...
      const trackedExpandedTopicId = getTrackedExpandedTopicId();
      const trackedExpandedTopicMode = trackedExpandedTopicId ? getTopicSummaryViewMode(trackedExpandedTopicId) : TOPIC_SUMMARY_VIEW_MODE.SUMMARY;
      ...
      topicItems.forEach((item) => {
        ...
        if (!autoExpandCandidateTopicId && state2.autoShowSummaryInList && hasLocalHistory && !isBusy) {
          autoExpandCandidateTopicId = topicId;
        }
      ...
      let targetExpandedTopicId = null;
      const preserveTrackedExpansion = trackedExpandedTopicId && !topicIdsOnPage.has(trackedExpandedTopicId) && shouldPreservePendingExpandedTopic(trackedExpandedTopicId);
      if (trackedExpandedTopicId && topicIdsOnPage.has(trackedExpandedTopicId)) {
        targetExpandedTopicId = trackedExpandedTopicId;
      } else if (state2.autoShowSummaryInList && autoExpandCandidateTopicId) {
        targetExpandedTopicId = autoExpandCandidateTopicId;
      }
      if (targetExpandedTopicId) {
        const expanded = expandSummaryRowByTopicId2(targetExpandedTopicId, {
          preferredMode: targetExpandedTopicId === trackedExpandedTopicId ? trackedExpandedTopicMode : getTopicSummaryViewMode(targetExpandedTopicId)
        });
        if (!expanded) {
          const shouldKeepTracking = targetExpandedTopicId === trackedExpandedTopicId && shouldPreservePendingExpandedTopic(trackedExpandedTopicId);
    ...
    function restoreExpandedSummaryRows() {
      const trackedTopicId = getTrackedExpandedTopicId();
      const shouldKeepTrackedExpansion = shouldPreservePendingExpandedTopic(trackedTopicId);
      if (trackedTopicId) {
        if (expandSummaryRowByTopicId2(trackedTopicId)) {
          return;
        }
        if (!shouldKeepTrackedExpansion) {
          setTrackedExpandedTopicId(null);
        }
      }
      if (!state2.autoShowSummaryInList) {
        collapseAllTopicSummaryViewsExcept(null, {
          preserveTracking: shouldKeepTrackedExpansion
        });
        return;
      }
      ...
      for (const button of buttons) {
        ...
        expandSummaryRowByTopicId2(topicId);
        return;
      }
      collapseAllTopicSummaryViewsExcept(null, {
        preserveTracking: shouldKeepTrackedExpansion
    ...
    function updateAllSummaryButtonsAndContainers() {
      ...
      let targetExpandedTopicId = trackedExpandedTopicId;
      const preserveTrackedExpansion = trackedExpandedTopicId && !topicIds.has(trackedExpandedTopicId) && shouldPreservePendingExpandedTopic(trackedExpandedTopicId);
      if (!targetExpandedTopicId || !topicIds.has(targetExpandedTopicId)) {
        targetExpandedTopicId = null;
      }
      if (!targetExpandedTopicId && state2.autoShowSummaryInList && autoExpandCandidateTopicId) {
        targetExpandedTopicId = autoExpandCandidateTopicId;
      }
      if (targetExpandedTopicId) {
        if (!expandSummaryRowByTopicId2(targetExpandedTopicId, {
          preferredMode: targetExpandedTopicId === trackedExpandedTopicId ? trackedExpandedTopicMode : getTopicSummaryViewMode(targetExpandedTopicId)
        })) {
          const shouldKeepTracking = targetExpandedTopicId === trackedExpandedTopicId && shouldPreservePendingExpandedTopic(trackedExpandedTopicId);
          if (!shouldKeepTracking) {
            ...
            <div class="tab-content" id="list-summary-settings">
                ...
                <div class="list-summary-sub-tab-panels">
                    <div class="list-summary-sub-tab-content settings-card active" id="list-summary-position">
                        ...
                        <hr>
                        <div class="switch-container">
                            <span class="switch-label">2. 自动展开全部历史总结（关/开）</span>
                            <label class="switch switch-on-off">
